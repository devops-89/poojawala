...
export default function BookingsContent() {
  ...
  const handleComplaintFileChange = (
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      ...
      setActiveBookings((prev) =>
        prev.filter(
          (job) => (job.originalData?.id || job.id) !== updatingJobId,
        ),
      );
  ...
  const handleRespondBooking = async (
    ...
    try {
      showLoader(
        action === "ACCEPT" ? "Accepting Booking..." : "Declining Booking...",
      );
      ...
      if (response.success) {
        ...
        showSnackbar(
          response.message || `Failed to ${action.toLowerCase()} booking`,
          "error",
        );
      }
    } catch (error: any) {
  ...
  useEffect(() => {
        ...
        if (response?.data?.bookings) {
          const getPriceDisplay = (booking: any) => {
            if (
              ...
              Number(booking.purohitPayoutAmount) > 0
            )
    ...
    const fetchActiveBookings = async () => {
      setIsFetchingBookings(true);
      try {
        const response = await getActiveBookingAPI(
          activeStatus,
          activePage,
          10,
        );
    ...
    <Box>
          ...
          <CustomTabPanel value={tabValue} index={1}>
            ...
            {isFetchingBookings ? (
              ...
              >
                ...
                (job) =>
                  job.status?.toUpperCase() === activeStatus.toUpperCase(),
              ).length === 0 ? (
              <Paper
                sx={{
                  p: 4,
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              try {
                                ...
                                const callbackUrl =
                                  "https://poojawala.com/purohit/bookings";
                                const res = await getPaymentLinkAPI(
                                  idToUse,
                                  callbackUrl,
                                );
                      ...
                      <Stepper
                        activeStep={
                          job.status?.toUpperCase() === "CANCELLED"
                            ...
                            : statuses.findIndex(
                                (s) =>
                                  s.toUpperCase() === job.status?.toUpperCase(),
                              )
                        }
                        alternativeLabel
                        sx={{
            ...
            onClick={async () => {
              if (otpValue.length === 6 && otpJobId) {
                try {
                  ...
                  showSnackbar(
                    "OTP verified successfully! Job started.",
                    "success",
                  );
                } catch (e) {
                  console.error(e);
      ...
      >
        ...
        <DialogContent dividers>
          ...
          >
            To mark this ritual as completed and trigger the payout, please
          ...
          <TextField
            fullWidth
            placeholder="Enter 6-digit OTP shared by client"
            variant="outlined"
          ...
          >
            2. Upload Ceremony Photo (Optional)
          </Typography>
          <Box
            sx={{
              border: "1px dashed #ccc",
          ...
          >
            ...
            >
              Click to upload a photo of the completed puja.
            </Typography>
          </Box>
          ...
          <TextField
            ...
            multiline
            rows={3}
            placeholder="Any notes regarding the ritual..."
            variant="outlined"
