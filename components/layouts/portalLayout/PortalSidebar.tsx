...
export default function PortalSidebar({ setMobileOpen }: PortalSidebarProps) {
  ...
  const menuItems = [
    ...
    { text: 'Pooja Bookings', icon: <AssignmentIcon />, path: '/purohit/bookings' },
    { text: 'Earnings', icon: <AccountBalanceWalletIcon />, path: '/purohit/earnings' },
    { text: 'My Services', icon: <LocalOfferIcon />, path: '/purohit/my-services' },
    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: '#ffffff', color: '#1e293b', borderRight: '1px solid #e2e8f0' }}>
      ...
      <List sx={{ flex: 1, px: 2, py: 3, '& .MuiListItemButton-root': { mb: 0.5, borderRadius: '12px' } }}>
        {menuItems.map((item: any) => {
          const isActive = pathname === item.path || (item.path === '/purohit/my-services' && pathname === '/purohit/services');
          return (
      ...
      >
        ...
        <Box sx={{ flex: 1 }}>
          <Typography sx={{ fontFamily: 'var(--font-outfit), sans-serif', fontWeight: 600, fontSize: '0.9rem', color: '#1e293b' }}>
            {profile?.firstName ? `${profile.firstName} ${profile.lastName || ''}` : 'Acharya V. Sharma'}
          </Typography>
      ...
      >
        <MenuItem component={NextLink} href="/purohit/profile" onClick={() => setAnchorEl(null)}>
          <ListItemIcon>
